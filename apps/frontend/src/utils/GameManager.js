...
export class GameManager {
  ...
  async initialize(canvas, spaceData, assets = {}) {
    ...
    this.initializationPromise = this._performInitialization(canvas, spaceData, assets);
    return this.initializationPromise;
  }
  ...
  connectWebSocket(wsConnection) {
    if (this.wsConnection) {
      console.warn("[GameManager] WebSocket already connected, ignoring duplicate connection");
      return;
    }
  ...
  async _processWebSocketMessage(message) {
    if (!this.scene) {
      console.warn("[GameManager] No scene available for message:", message.type);
      return;
    }
    ...
    switch (message.type) {
      case "space-joined":
        if (this.connectionState === "JOINED") {
          console.warn("[GameManager] Already joined! Ignoring duplicate space-joined message");
          return;
        }
        if (this.userId) {
          console.warn("[GameManager] Already have userId! Ignoring duplicate space-joined message");
          return;
        }
  ...
  async _createRemoteUser(userData) {
    ...
    try {
      ...
      const existingUsers = this.scene.children.filter(child => child.userId === userData.userId);
  ...
  async _handleMovement(payload) {
    ...
    if (remoteUser && remoteUser.updatePosition) {
      ...
      console.warn("[GameManager] Could not find remote user for movement:", payload.userId);
    }
  }
